...
class BERTWrapper(tf.keras.Model):
    def __init__(self, bert: BERT, config):
        super(BERTWrapper, self).__init__(name='BERTWrapper')
        self.bert = bert
        self.final_linear = tf.keras.layers.Dense(100)
    ...
    def call(self, input, pad_seg_embed_token, training, enc_padding_mask):
        ...
        cls_token_vec = self.final_linear(self.encoder_output)[:,0]
    ...
    def get_assignment_map_from_checkpoint(self, tvars, init_checkpoint):
        ...
        for x in init_vars:
            ...
            #             name_index = [i for i, _ in enumerate(name) if _=='/'][-1]
            #             new_scope_index = [i for i, _ in enumerate(new_scope_name) if _=='/'][-1]
            #             assignment_map[name[:name_index]+'/'] = new_scope_name[:new_scope_index]+'/'
...
def main():
    ...
    bert_wrapper = BERTWrapper(bert, config)
    ...
    masked_training_ids_batch, mask_MLM_position_index_batch, mask_MLM_token_ids_batch = bert_dataset.create_LM_mask(
        training_ids_batch, sp)
    ...
    nsp_sent_ids_batch, nsp_sparse_label_batch, integrated_mask_LM_position_index_batch, seg_embed_token_batch = bert_dataset.create_nsp_MLM_dataset(
        masked_training_ids_batch, mask_MLM_label_batch, mask_MLM_position_index_batch, sp)
    ...
    sample_num = 10
    X_train = tf.convert_to_tensor(np.array(pad_nsp_sent_ids_batch)[:sample_num])
    tar_real = tf.convert_to_tensor(np.array(pad_nsp_sparse_label_batch)[:sample_num])
    pad_seg_embed_token_batch = tf.convert_to_tensor(np.array(pad_seg_embed_token_batch)[:sample_num])
    enc_padding_mask = tf.convert_to_tensor(bert_dataset.create_masks(X_train)[:sample_num])

    encoder_output = bert_wrapper(X_train, pad_seg_embed_token=pad_seg_embed_token_batch,
                                  training=True, enc_padding_mask=enc_padding_mask)

    print("before, bert_wrapper.trainable_variables: ", bert_wrapper.trainable_variables)

    checkpoint_dir = '../save_model_estimator'
    init_checkpoint = tf.train.latest_checkpoint(checkpoint_dir)

    init_vars = tf.train.list_variables(init_checkpoint)
    print(init_vars)

    tvars = bert_wrapper.trainable_variables
    (assignment_map, initialized_variable_names) = bert_wrapper.get_assignment_map_from_checkpoint(tvars,
                                                                                                   init_checkpoint)
